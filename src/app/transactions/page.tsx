    ...
    <StandalonePageShell maxWidth="5xl">
      ...
      <div className="space-y-4">
        {transactions.map((tx, index) => (
          <article
            key={tx.hash}
            className="min-w-0 rounded-[24px] border border-orbit-border bg-orbit-surface-strong p-4 sm:p-5"
          >
            <div className="flex min-w-0 items-start gap-3 sm:items-center sm:gap-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-sm text-orbit-muted">{tx.time}</p>
                <p className="mt-1 break-all font-mono text-xs text-orbit-accent sm:text-sm">
                  {tx.hash}
                </p>
              </div>
            </div>
